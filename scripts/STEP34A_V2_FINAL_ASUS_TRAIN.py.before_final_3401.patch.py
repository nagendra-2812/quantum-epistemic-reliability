...
print()
print("=" * 100)
print("STEP 34A-v2 — FINAL ASUS CBIS RESNET-50 PUBLICATION RUN")
print("=" * 100)
...
if len(failure_df) > 0:
    ...
    print()
    print(
        "WARNING: some records could not be selected."
    )
    ...
    raise RuntimeError(
        "Input selection is incomplete. "
        "Training was NOT started."
    )


if len(selected_df) != len(
    available
):

    raise RuntimeError(
        "Selected input count does not equal available "
        "cohort count. Training was NOT started."
    )
...
readme = f"""
STEP 34A-v2 — ASUS LOCAL CBIS-DDSM PUBLICATION RUN
